init:
    python:
        ...
        config.thumbnail_width = scale_x(318)
        config.thumbnail_height = scale_y(179)
        ...
        style.file_picker_text.size = scale_y(150)
        ...
        style.chapter.size = scale_y(120)
...
screen main_menu:
    ...
    $ owl_played = 1
    imagemap:
        auto get_image("gui/title_menu/mainmenu_%s.png")
        hotspot ((scale_x(439), scale_y(265), scale_x(318), scale_y(621))):
            clicked Start()
        hotspot ((scale_x(787), scale_y(261), scale_x(270), scale_y(537))):
            clicked ShowMenu('load')
        hotspot ((scale_x(1067), scale_y(748), scale_x(252), scale_y(312))):
            clicked ShowMenu('preferences')
        hotspot ((scale_x(1083), scale_y(258), scale_x(229), scale_y(538))):
            clicked ShowMenu('gallery')
        hotspot ((scale_x(1459), scale_y(532), scale_x(149), scale_y(295))):
            clicked ShowMenu('quit')
            hovered (Play('test_one', 'sound/sfx/menu_gate.ogg'))
        hotspot ((scale_x(170), scale_y(609), scale_x(111), scale_y(152))):
...
screen game_menu_selector:
    ...
    imagemap:
        ...
        hotspot ((scale_x(630), scale_y(390), scale_x(660), scale_y(70))):
            clicked MainMenu()
        hotspot ((scale_x(630), scale_y(455), scale_x(660), scale_y(70))):
            clicked ShowMenu('load')
        hotspot ((scale_x(630), scale_y(520), scale_x(660), scale_y(70))):
            clicked ShowMenu('preferences')
        hotspot ((scale_x(630), scale_y(585), scale_x(660), scale_y(70))):
            clicked ShowMenu('save')
        hotspot ((scale_x(630), scale_y(650), scale_x(660), scale_y(70))):
            clicked Return()
            ...
            for i in range(0, 10):
                if (i == 0):
                    ...
                    textbutton str(i):
                        ...
                        text_size scale_y(40)
        window:
            right_padding scale_x(200)
            bottom_padding scale_y(160)
            top_padding scale_y(130)
            left_padding scale_x(230)
            background None
            grid 4 3:
                transpose True
                xfill True
                    ...
                    button:
                        ...
                        vbox:
                            add FileScreenshot(i):
                                xpos 3
                                ypos 8
                            text (FileTime(i, format='%d.%m.%y, %H:%M', empty=_('Empty Slot.')) + '\n' + ' ' + FileSaveName(i)):
                                style 'log_button_text'
                                xpos -4
                                ypos 15
                                size scale_y(22)
...
screen preferences:
    tag menu
    modal False
    $ bar_null = Frame(get_image('gui/settings/bar_null.png'), scale_x(12), scale_y(12))
    $ bar_full = Frame(get_image('gui/settings/bar_full.png'), scale_x(12), scale_y(12))
    window:
...
screen choice:
    ...
    window:
        right_padding scale_x(75)
        bottom_padding scale_y(50)
        yalign 0.5
        top_padding scale_y(50)
        xfill True
        background (Frame(get_image('gui/choice/' + persistent.timeofday + '/choice_box.png'), scale_x(50), scale_y(50)))
        left_padding scale_x(75)
        vbox:
            xalign 0.5
...
screen nvl:
    ...
    $ timeofday = persistent.timeofday
    window:
        right_padding scale_x(175)
        xfill True
        yalign 0.5
        top_padding scale_y(150)
        background (Frame(get_image('gui/choice/' + timeofday + '/choice_box.png'), scale_x(50), scale_y(50)))
        yfill True
        left_padding scale_x(175)
        bottom_padding scale_y(150)
        vbox:
        ...
        for i, gallery_line in enumerate(gallery_grid[persistent.gallery_mode][gallery_min:gallery_max]):
            for j, gallery_item in enumerate(gallery_line):
                ...
                if ((persistent.gallery_mode, gallery_item) in persistent._seen_images):
                    python:
                        _t = im.Crop(ImageReference((persistent.gallery_mode, gallery_item)), (0, 0, persistent.resolution_x, persistent.resolution_y))
                        th = im.Scale(_t, scale_x(320), scale_y(180))
                        img = im.Composite((scale_x(336), scale_y(196)), (8, 8), im.Alpha(th, 0.5), (0, 0), im.Image(get_image('gui/gallery/thumbnail_idle.png')))
                        imgh = im.Composite((scale_x(336), scale_y(196)), (8, 8), th, (0, 0), im.Image(get_image('gui/gallery/thumbnail_hover.png')))
                    imagebutton:
                        xalign x
                        hover imgh
                        yalign y
        ...
        for j, gallery_item in enumerate(gallery_grid[persistent.gallery_mode][gallery_min]):
            ...
            if ((gallery_item[0] in persistent._seen_images) or (tuple(gallery_item[0].split(' ')) in persistent._seen_images)):
                window:
                    xalign x
                    yalign y
                    ymaximum scale_y(750)
                    top_padding 0
                    xmaximum scale_x(270)
                    background Solid('#ffffff80')
                    left_padding 0
                    add ImageReference(gallery_item[0]):
                        zoom 0.69
                        crop ((scale_x(250), 0, scale_x(390), persistent.resolution_y))
                        ypos 7
                python:
...
label _compat_confirm_quit:
    python:
        ...
        ex_map_points = (
                        (scale_x(980), scale_y(520), scale_x(1150), scale_y(640), True),
                        (scale_x(1400), scale_y(520), scale_x(1570), scale_y(640), False),
                        )
...
init -3:
    python:
        ...
        '''style.readback_window.xmaximum = scale_x(760)
            style.readback_window.ymaximum = scale_y(500)
...
screen text_history:
    ...
    $ adj = NewAdj(changed=store_yvalue, step=scale_x(300))
    window:
                            ...
                            if (not line[2]):
                                ...
                                if (persistent.font_size == 'small'):
                                    textbutton line[1]:
                                        ...
                                        text_size scale_y(28)
                                elif (persistent.font_size == 'large'):
                                    textbutton line[1]:
                                        style 'log_button'
                                        text_style 'log_button_text'
                                        action (FunctionCallback(do_rollback, cn))
                                        text_size scale_y(35)
                                else:
                                    null:
                                        height 1
                            else:
