init -999:
    ...
    image backdrop_back = get_image("anim/backdrop/back.jpg")
    image backdrop_new:
        pause 0.1 
        get_image("anim/backdrop/1.png") 
        pause 0.1 
        get_image("anim/backdrop/2.png") 
        pause 0.1 
        get_image("anim/backdrop/3.png") 
        pause 0.1 
        get_image("anim/backdrop/2.png") 
        repeat
    ...
    $ style.backdrop_text.size = scale_y(80)
init 5:
    python:
            ...
            def on_load_callback(slot):
                try:
                    if persistent.on_save_timeofday[slot]:
                        ...
                        persistent.hentai = True #persistent.on_save_timeofday[slot][3]
...
label splashscreen:
    ...
    if hour in [22,23,24,0,1,2,3,4,5,6]:
        ...
        scene splashscreen_night:
            pos (0,0) 
            linear 4.0 pos (0,-persistent.resolution_y) 
        with dissolve
        $ renpy.pause(4)
        with None
        show logo_night:
            pos (scale_x(400),scale_y(150)) 
        with dissolve2
        $ renpy.pause(3)
        return
