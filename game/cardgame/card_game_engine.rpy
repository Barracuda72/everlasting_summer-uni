init 2:
    python:
        ...
        if  not config_session:
            ...
            screen_width = persistent.resolution_x
            screen_height = persistent.resolution_y
            ...
            prefix  = get_image("cards/")
            types   = ["2ch","ussr","utan","uvao"]
            suffix  = ".png"
            card_height    = scale_y(315)
            card_width     = scale_x(210)
            card_dx        = scale_x(225)
            card_left_dx   = scale_x(50)
            card_top_dy    = scale_y(50)
            button_dx      = 0
            button_dy      = 0
            bottom_dy      = 0
            bottom_dy_result = 0
            ...
            arrow_width = scale_x(50)
            
            ints = range(1,14)
            card_img={}
            card_img["cover"] = get_img("cover")
            card_img[name_of_none] = get_image("misc/none.png")
    ...
    python:
        ...
        def show_cards():
            ...
            ui.imagebutton(avatar,avatar,clicked=None,
                          xpos=xst+card_dx*(n_cards)+card_left_dx+step,
                          ypos=card_top_dy,
                      )
            ui.button(clicked=None, xanchor=1.0, xpadding=6, xminimum=xmin,
                          xpos=xst+card_dx*(n_cards+1)+card_left_dx+button_dx,
                          ypos=card_top_dy+yst,
                          style=style.cards_button,
                      )
            yst += step
            ui.text(u"Соперник:",style="button_text",size=scale_y(15))
            ui.button(clicked=None, xanchor=1.0, xpadding=6, xminimum=xmin,
                          xpos=xst+card_dx*(n_cards+1)+card_left_dx+button_dx,
                          ypos=card_top_dy+yst,
                          style=style.cards_button,
            ...
            yst += step
            ui.text(rival.name,style="button_text",size=scale_y(25))
            ...
            yst += step
            ui.text(u"Чей ход:",style="button_text",size=scale_y(15))
            ui.button(clicked=None, xanchor=1.0, xpadding=6, xminimum=xmin,
                          xpos=xst+card_dx*(n_cards+1)+card_left_dx+button_dx,
                          ypos=card_top_dy+yst,
                          style=style.cards_button,
            ...
            if  cards_state in ["me_defend_1","me_defend_2","me_get","me_select_1","me_select_2"]:
                ui.text(u"Твой",style="button_text",size=scale_y(25))
            if  cards_state in ["rival_defend","rival_get","rival_select"]:
                ui.text(u"Чужой",style="button_text",size=scale_y(25))
            if  cards_state in ["results"]:
                ui.text(u"---",style="button_text",size=scale_y(25))
            ...
            yst += step
            ui.text(u"Фаза игры:",style="button_text",size=scale_y(15))
            ui.button(clicked=None, xanchor=1.0, xpadding=6, xminimum=xmin,
                          xpos=xst+card_dx*(n_cards+1)+card_left_dx+button_dx,
                          ypos=card_top_dy+yst,
                          style=style.cards_button,
            ...
            if  cards_state in ["me_defend_1","me_defend_2","rival_defend"]:
                ui.text(u"Защита",style="button_text",size=scale_y(25))
            if  cards_state == "me_select_1":
                ui.text(u"Сброс",style="button_text",size=scale_y(25))
            if  cards_state in ["me_select_2","rival_select"]:
                ui.text(u"Захват",style="button_text",size=scale_y(25))
            if  cards_state in ["me_get","rival_get"]:
                if  CARD_GAME_WITH_EXCHANGE:
                    ui.text(u"Обмен",style="button_text",size=scale_y(25))
                else:
                    ui.text(u"Вытягивание",style="button_text",size=scale_y(25))
            if  cards_state in ["results"]:
                ui.text(u"Итоги",style="button_text",size=scale_y(25))
            
            if  cards_state in ["me_defend_1","me_defend_2"]:
                ui.button(clicked=ui.returns("end_of_turn"), xanchor=1.0,
                              xpadding=6, xminimum=scale_x(227),
                              xpos=scale_x(227), #card_dx*(n_cards+1)+card_left_dx+button_dx,
                              ypos=(card_bottom_dy+card_top_dy)/2 + button_dy,
                              style=style.cards_button,
                          )
                ui.text(u"X",style="button_text",size=scale_y(100))
            ...
            yst += step*2
            ui.text("%d"%cycles_left,style="button_text",size=scale_y(25))
            ...
            yst += step
            ui.text(u"Обменов осталось:",style="button_text",size=scale_y(15))
            ui.button(clicked=None, xanchor=1.0, xpadding=6, xminimum=xmin,
                          xpos=xst+card_dx*(n_cards+1)+card_left_dx+button_dx,
                          ypos=card_top_dy+yst,
                          style=style.cards_button,
            ...
            ui.text(changes_left_text,style="button_text",size=scale_y(25))
            
            if  result_status != 'in_progress':
                renpy.block_rollback()
                ui.button(clicked=ui.returns("ok"), xanchor=1.0, xpadding=6, xminimum=xmin,
                          xpos=xst+screen_width/2+100,
                          ypos=screen_height/2-35 - bottom_dy - bottom_dy_result,
                          style=style.cards_button,
                      )
                if  result_status == 'win':
                    ui.text(u"Победа",style="button_text",size=scale_y(72))
                if  result_status == 'draw':
                    ui.text(u"Ничья",style="button_text",size=scale_y(72))
                if  result_status == 'fail':
                    ui.text(u"FAIL",style="button_text",size=scale_y(72))
